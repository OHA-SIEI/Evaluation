...
SIMSdatain$SNU1.Name[str_detect(SIMSdatain$SNU1.Name, "Mohale’s Hoek") == TRUE] <- "Mohale’s Hoek"
SIMSdatain$SNU1.Name[str_detect(SIMSdatain$SNU1.Name, "Mafeteng") == TRUE] <- "Mafeteng"
...
rename(SIMSdatain, CEE.Number.and.Name2 = CEE.Number.and.Name)
SIMSdatain$CEE.Number.and.Name <- paste0(SIMSdatain$CEE.Name, " (", SIMSdatain$CEE.Number, ")")

SIMSdatain$CEE.Number.and.Name[str_detect(SIMSdatain$CEE.Number.and.Name, "3.05") == TRUE] <- "Ability to Produce KP-specific Program Data  (3.05)"
SIMSdatain$CEE.Number.and.Name[str_detect(SIMSdatain$CEE.Number.and.Name, "8.02") == TRUE] <- "ART Provision for HIV-Positive Adult TB Patients (8.02)"
SIMSdatain$CEE.Number.and.Name[str_detect(SIMSdatain$CEE.Number.and.Name, "1.05") == TRUE] <- "Data Collection/Review (1.05)"
SIMSdatain$CEE.Number.and.Name[str_detect(SIMSdatain$CEE.Number.and.Name, "1.19") == TRUE] <- "Data Quality Assurance(Routine Activities) (1.19)"

SIMSdatain$CEE.Number.and.Name[str_detect(SIMSdatain$CEE.Number.and.Name, "1.22") == TRUE] <- "Data Reporting Consistency -HTS_TST (1.22)"
SIMSdatain$CEE.Number.and.Name[str_detect(SIMSdatain$CEE.Number.and.Name, "1.23") == TRUE] <- "Data Reporting Consistency -PMTCT_STAT (1.23)"
SIMSdatain$CEE.Number.and.Name[str_detect(SIMSdatain$CEE.Number.and.Name, "1.24") == TRUE] <- "Data Reporting Consistency -VMMC_CIRC (1.24)"
SIMSdatain$CEE.Number.and.Name[str_detect(SIMSdatain$CEE.Number.and.Name, "1.24") == TRUE] <- "Data Reporting Consistency -VMMC_CIRC (1.24)"

SIMSdatain$CEE.Number.and.Name[str_detect(SIMSdatain$CEE.Number.and.Name, "7.08") == TRUE] <- "Index Testing Training and Supportive Supervision (7.08)"
SIMSdatain$CEE.Number.and.Name[str_detect(SIMSdatain$CEE.Number.and.Name, "7.11") == TRUE] <- "Intimate Partner Violence Risk Assessment and Support (7.11)"

SIMSdatain$CEE.Number.and.Name[str_detect(SIMSdatain$CEE.Number.and.Name, "1.03") == TRUE] <- "Management and Planning - Operational Planning  (1.03)"
SIMSdatain$CEE.Number.and.Name[str_detect(SIMSdatain$CEE.Number.and.Name, "1.03") == TRUE] <- "Management and Planning - Operational Planning  (1.03)"

SIMSdatain$CEE.Number.and.Name[str_detect(SIMSdatain$CEE.Number.and.Name, "4.07") == TRUE] <- "Partner Services (4.07)"

SIMSdatain$CEE.Number.and.Name[str_detect(SIMSdatain$CEE.Number.and.Name, "4.13") == TRUE] <- "PITC for Maternity Patients (4.13)"

SIMSdatain$CEE.Number.and.Name[str_detect(SIMSdatain$CEE.Number.and.Name, "3.15") == TRUE] <- "Routine HIV Testing of Biological Children of Adult Clients  (3.15)"
SIMSdatain$CEE.Number.and.Name[str_detect(SIMSdatain$CEE.Number.and.Name, "4.08") == TRUE] <- "Routine HIV testing of Children of Adult Patients (4.08)"
SIMSdatain$CEE.Number.and.Name[str_detect(SIMSdatain$CEE.Number.and.Name, "2.08") == TRUE] <- "Routine HIV Testing of Children of Adult Patients (2.08)"

SIMSdatain$CEE.Number.and.Name[str_detect(SIMSdatain$CEE.Number.and.Name, "8.01") == TRUE] <- "Routine PITC for Patients with TB and Presumptive TB (8.01)"

SIMSdatain$CEE.Number.and.Name[str_detect(SIMSdatain$CEE.Number.and.Name, "2.14") == TRUE] <- "Service Referral and Linkage System (2.14)"

SIMSdatain$CEE.Number.and.Name[str_detect(SIMSdatain$CEE.Number.and.Name, "2.32") == TRUE] <- "Service Referral and Linkage System (2.32)"

SIMSdatain$CEE.Number.and.Name[str_detect(SIMSdatain$CEE.Number.and.Name, "1.04") == TRUE] <- "Supervision -Health Sector (1.04)"

SIMSdatain$CEE.Number.and.Name[str_detect(SIMSdatain$CEE.Number.and.Name, "1.17") == TRUE] <- "Supply Chain Reliability-Pediatric Isoniazid Preventive Therapy/TB Preventive Therapy/Rifapentine (1.17)"

SIMSdatain$CEE.Number.and.Name[str_detect(SIMSdatain$CEE.Number.and.Name, "2.3") == TRUE] <- "Support Services for HIV-Positive Adolescents (2.3)"

SIMSdatain$CEE.Number.and.Name[str_detect(SIMSdatain$CEE.Number.and.Name, "4.06") == TRUE] <- "Support Services for HIV-Positive Pregnant Adolescents in ANC (4.06)"

SIMSdatain$CEE.Number.and.Name[str_detect(SIMSdatain$CEE.Number.and.Name, "10.03") == TRUE] <- "Test SOP (10.03)"

SIMSdatain$CEE.Number.and.Name[str_detect(SIMSdatain$CEE.Number.and.Name, "1.02") == TRUE] <- "Use of Data from Health Economics and Finance studies  (1.02)"

SIMSdatain$CEE.Number.and.Name[str_detect(SIMSdatain$CEE.Number.and.Name, "2.21") == TRUE] <- "Dosing of Pediatric and Adolescent ARVs (2.21)"


SIMSdatain$Set.Number.and.Name[str_detect(SIMSdatain$Set.Number.and.Name, "1.") == TRUE] <- "1. HIV Planning, Coordination and Management-Health"
SIMSdatain$Set.Number.and.Name[str_detect(SIMSdatain$Set.Number.and.Name, "3A.") == TRUE] <- "3A. Key Populations-General"
SIMSdatain$Set.Number.and.Name[str_detect(SIMSdatain$Set.Number.and.Name, "3B.") == TRUE] <- "3B. Care and Treatment-Key Populations"
SIMSdatain$Set.Number.and.Name[str_detect(SIMSdatain$Set.Number.and.Name, "4.") == TRUE] <- "4. Private Sector Engagement and Advocacy"
SIMSdatain$Set.Number.and.Name[str_detect(SIMSdatain$Set.Number.and.Name, "4A.") == TRUE] <- "4A. PMTCT, Postnatal, and L&D"
SIMSdatain$Set.Number.and.Name[str_detect(SIMSdatain$Set.Number.and.Name, "5.") == TRUE] <- "5. Human Resources for Health"
SIMSdatain$Set.Number.and.Name[str_detect(SIMSdatain$Set.Number.and.Name, "8.") == TRUE] <- "8. Laboratory and Blood Transfusion Support"
...
SIMSdata_filtered <- filter(SIMSdata, 
                            #OU.Name == "Lesotho")
...
SIMSdata_filtered2 <- SIMSdata_filtered %>%
  mutate(Follow.Up.Date.Deadline2 = mdy(Follow.Up.Date.Deadline)) %>%
  mutate(Assessment.Date2 = as.Date(Assessment.Date, "%m/%d/20%y")) %>%
  mutate(FY22followup = ifelse(Follow.Up.Date.Deadline2 >=as.Date("2021-10-01") &  Follow.Up.Date.Deadline2 <= as.Date("2022-09-30"), 1, 0)) %>% #I'm checking to see if there is any clean/easy way to get completed/missed/upcoming####
filter(if(SiteOnly==1) Tool.Type == "Site" else TRUE) ##Filtering to Site Only
...
SIMSdata_visits  <- SIMSdata_filtered2 %>% 
  select("SNU1.Name", "orgUnituid", "Assessment.ID", "DateAssessment") %>% 
  distinct %>%
...
# SIMSdata2 <- SIMSdata1 %>%
#   mutate(AssessmentStatus = case_when())
...
#####Looking at the Set level#####
SIMSdataSetwithoutdups <- data.frame(aggregate(list(Score.for.Green =  SIMSdatafiltered$Score.for.Green,
                                                       ...
                                                       Countnodups = SIMSdatafiltered$X1), by=list(Set.Number.and.Name = 
                                                                                                     SIMSdatafiltered$Set.Number.and.Name), FUN = sum))
...
SIMSdataSetwithoutdups$PercentGreen <- 100*(SIMSdataSetwithoutdups$Score.for.Green / SIMSdataSetwithoutdups$Countnodups)
SIMSdataSetwithoutdups$PercentRed <- 100*(SIMSdataSetwithoutdups$Score.for.Red / SIMSdataSetwithoutdups$Countnodups)
SIMSdataSetwithoutdups$PercentRedYellow <- 100*(SIMSdataSetwithoutdups$Score.for.Red...Yellow / SIMSdataSetwithoutdups$Countnodups)
...
mostegregiousSet <-   head( SIMSdataSetwithoutdups[(order(SIMSdataSetwithoutdups$PercentRed,
                 SIMSdataSetwithoutdups$Countnodups, decreasing = TRUE)),], n=1)
...
worst <- mostegregioustographlong %>% ggplot(aes(x=CEE.Number.and.Name, y = Percent.Breakdown, fill=
  ...
  scale_fill_manual(values = c("#287c67",  "#f2bc40", "#ba0c2f"))
...
bestscoringSNU <-   SIMSdataSNUwithoutdups[(order(SIMSdataSNUwithoutdups$PercentGreen, 
                                                           SIMSdataSNUwithoutdups$Countnodups, decreasing = TRUE)),]
...
bestSNU <- bestscoringSNUtographlong %>% ggplot(aes(x=SNU1.Name, y = Percent.Breakdown, fill=
                                                factor(Scores, levels=c("Green", "Yellow", "Red")))) +
  geom_bar(position = "fill", stat = "identity")+
  ggtitle("CEE scores over SNUs")+xlab('SNUs')+ylab('Score breakdown between green, yellow, and red scores') +
  theme(plot.title = element_text(hjust = 0.5)) + coord_flip() +  
  scale_fill_manual(values = c("#287c67",  "#f2bc40", "#ba0c2f"))
...
bestscoringpartner <-   SIMSdatapartnerwithoutdups[(order(SIMSdatapartnerwithoutdups$PercentGreen, 
                                                                  SIMSdatapartnerwithoutdups$Countnodups, decreasing = TRUE)),]
...
bestpartner <- bestscoringpartnerographlong %>% ggplot(aes(x=PrimePartner, y = Percent.Breakdown, fill=
                                                      factor(Scores, levels=c("Green", "Yellow", "Red")))) +
  geom_bar(position = "fill", stat = "identity")+
  ggtitle("CEE scores over implementing partners")+xlab('Partners')+ylab('Score breakdown between green, yellow, and red scores') +
  theme(plot.title = element_text(hjust = 0.5)) + coord_flip() +  
  scale_fill_manual(values = c("#287c67",  "#f2bc40", "#ba0c2f"))
worstpartner <- worstscoringpartnertographlong %>% ggplot(aes(x=PrimePartner, y = Percent.Breakdown, fill=
  ...
  theme(plot.title = element_text(hjust = 0.5)) + coord_flip() +  
  #scale_color_si("genoas", discrete = TRUE)
  scale_fill_manual(values = c("#287c67",  "#f2bc40", "#ba0c2f")) 
```
...
The purpose of this document is to aid identification of data and program issues that can be identified through SIMS assessments, with the view of raising these issues with the country team so that any problems can be resolved.  These analyses would be performed on a regular basis and could be further incorporated into POART discussions.   
...
In FY22, `r comp_n22` comprehensive, and `r FU_n22` follow-up, assessments were completed in Sites in `r SIMSdatafiltered$OU.Name[1]`. Further filtering on SIMS data (by dates, location, partner, and more) can be conducted in the [USAID SIMS OU Dashboard](https://tableau.usaid.gov/#/views/USAIDSIMSDashboard/OUPerformanceSummary); this dashboard provides an initial overview of the SIMS assessments in the country and is updated on a daily basis (compared to what is in Panorama which is updated quarterly and is available to a wider audience).

This note is being generated for all OUs; therefore, some analyses may need further clarity due to both the nature of the global analysis being applied to all data as well as a necessity for further data cleaning. It is intended to be a living document (which can be generated at an ongoing level) which can be customized or corrected on request (assuming the level of effort is kept to a reasonable level); please reach out to [internal.oha.evaluation@usaid.gov](mailto:internal.oha.evaluation@usaid.gov) for more information.


## Bottom CEE Scores in `r SIMSdatafiltered$OU.Name[1]`

The worst-performing set in the OU (with scores which were later re-scored at the same facilities removed) is **`r mostegregiousSet[1,1]`**, for which `r round(mostegregiousSet[1,13], digits=2)`% of scores were red (and `r round(mostegregiousSet[1,12], digits=2)`% were green).The overall bottom 5 CEEs in the OU (with scores which were later re-scored at the same facilities removed, and only keeping CEEs which were assessed by five or more sites) include:
...
*Flag for discussion with the OU cluster team: What are some of the plans you have for addressing some of these poorer-performing CEEs at Sites? What about the poor-performing set, `r mostegregiousSet[1,1]`?*
