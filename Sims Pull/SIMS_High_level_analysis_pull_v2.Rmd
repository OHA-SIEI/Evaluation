---
title: <span style="font-size:125%; font-family:Sans Source">SIMS Summary params:OU.Name FY22 </span>
...
bestscoringSNU <-   head(SIMSdataSNUwithoutdups[(order(SIMSdataSNUwithoutdups$PercentGreen, 
                                                           SIMSdataSNUwithoutdups$Countnodups, decreasing = TRUE)),], n=5)
...
paste(/This report summarizes key findings from the last FY of completed (Site) SIMS assessments. In FY22,",comp_n22, 
      "comprehensive, and",FU_n22, "follow-up, assessments were completed in Sites in the OU."")

```

## Bottom CEE Scores in the OU

The overall bottom 5 CEEs in the OU (with scores which were later re-scored at the same facilities removed, and only keeping CEEs which were assessed by ten or more sites) include:
...
## CEE scores in the OU which have declined or not improved
...
Regions with larger shares of red CEEs indicate that those regions may have more sites which need improvement and/or re-assessment.  The overall bottom 5 SNUs in the OU (with scores which were later re-scored at the same facilities removed) are (in decreasing order of number of CEEs assessed in the SNU):
...
IPs with large shares of green CEEs indicate that those partners are doing well and perhaps could be models of best practice for other partners.  The overall top 3 IPs in the OU (with scores which were later re-scored at the same facilities removed) are:
