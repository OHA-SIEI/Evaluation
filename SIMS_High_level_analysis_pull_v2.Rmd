---
title: <span style="font-size:125%; font-family:sans-serif">SIMS Summary params:OU.Name FY22 </span> 
header-includes:
...
SIMSdata_filtered <- filter(SIMSdata, params$OU.Name)
                            #OU.Name == "Lesotho")
                            #  params$OU.Name)
...
# SIMS Summary for `r SIMSdatain$OU.Name[1]`


This report summarizes key findings from the last FY of completed (Site) SIMS assessments. In FY22, `r comp_n22` comprehensive, and `r FU_n22` follow-up, assessments were completed in Sites in `r SIMSdatain$OU.Name[1]`.


## Bottom CEE Scores in `r SIMSdatain$OU.Name[1]`
...
worst + theme(legend.position = "none", axis.text = element_text(size = 18), title = element_text(size = 20), axis.title = element_text(size = 19))  

```

## Top Scores in `r SIMSdatain$OU.Name[1]`
...
```{r echo=FALSE, fig.width=17, fig.height=9}
#, fig.width=14, fig.height=9}
best + theme(legend.position = "none", axis.text = element_text(size = 18), title = element_text(size = 20), axis.title = element_text(size = 19))  
```
...
1. `r mostdeclinegraph[1,1]` (scored in `r mostdecline[1,12]` sites; `r mostdecline[1,18]*100`% declined and `r mostdecline[1,20]*100`% not changed.)
2. `r mostdeclinegraph[2,1]` (scored in `r mostdecline[2,12]` sites; `r mostdecline[2,18]*100`% declined and `r mostdecline[2,20]*100`% not changed.)
3. `r mostdeclinegraph[3,1]` (scored in `r mostdecline[3,12]` sites; `r mostdecline[3,18]*100`% declined and `r mostdecline[3,20]*100`% not changed.)
4. `r mostdeclinegraph[4,1]` (scored in `r mostdecline[4,12]` sites; `r mostdecline[4,18]*100`% declined and `r mostdecline[4,20]*100`% not changed.)
5. `r mostdeclinegraph[5,1]` (scored in `r mostdecline[5,12]` sites; `r mostdecline[5,18]*100`% declined and `r mostdecline[5,20]*100`% not changed.)
...
Regions with large shares of green CEEs indicate that those regions are doing well and perhaps could be models of best practice for other sites.  The overall top 4 SNUs in the OU (with scores which were later re-scored at the same facilities removed) are (in decreasing order of number of CEEs assessed in the SNU):
...
4. `r bestscoringSNUgraph[4,1]` (`r bestscoringSNU[4,13]` CEEs scored)
...
Regions with larger shares of red CEEs indicate that those regions may have more sites which need improvement and/or re-assessment.  The overall bottom 4 SNUs in `r SIMSdatain$OU.Name[1]` (with scores which were later re-scored at the same facilities removed) are (in decreasing order of number of CEEs assessed in the SNU):
...
4. `r worstscoringSNUgraph[4,1]` (`r worstscoringSNU[4,13]` CEEs scored)
...
IPs with large shares of green CEEs indicate that those partners are doing well and perhaps could be models of best practice for other partners.  The overall top 3 IPs in `r SIMSdatain$OU.Name[1]` (with scores which were later re-scored at the same facilities removed) are:
...
3. `r bestscoringpartnergraph[3,1]` (`r bestscoringpartner[3,13]` CEEs scored)
...
IPs with larger shares of red CEEs indicate that those partners may have more sites which need improvement and/or re-assessment.  The overall bottom 3 IPs in the OU (with scores which were later re-scored at the same facilities removed) are:
...
3. `r worstscoringpartnergraph[3,1]` (`r worstscoringpartner[3,13]` CEEs scored)
